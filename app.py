...
# 3. FULL TICKER LIST & TIMELINES (RESTORED)
...
.stock-card { background: var(--bg2); border: 1px solid var(--border); border-radius: 6px; padding: 15px; margin-bottom:10px; cursor: pointer; height: 100px; }
...
# 5. RISK GATEWAY (STAYING AS A CENTERED BOX)
if not st.session_state.disclaimer_accepted:
    ...
    with center:
        ...
        with st.container(border=True):
            ...
            st.write("Predictions are statistical models and do not guarantee results. Use for informational purposes only.")
    ...
    for i, (key, cfg) in enumerate(TIME_SETTINGS.items()):
        with tabs[i]:
            ...
            if not df.empty:
                ...
                # 2. Main Chart (Candlestick or Accuracy Line)
                if show_acc and not df_day.empty:
                    ...
                    fig.update_layout(template="plotly_dark", height=450, title=f"{ticker} Actual vs Predicted (Today)")
                else:
